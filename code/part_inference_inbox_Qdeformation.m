function [detection_loc , ap_scores , sp_scores] = part_inference_inbox_Qdeformation(im_current, model_selected, bbox_current,root_bbox,deform_param_patch,quadratic_flg)
...
%          showboxes(im_current,top_boxes)
   ...
   detected_bbox = [detection_loc{m}(1) , detection_loc{m}(2) , detection_loc{m}(3)-detection_loc{m}(1) , detection_loc{m}(4)-detection_loc{m}(2)];
   anchor_bbox = [bbox_current{m}(1) , bbox_current{m}(2) , bbox_current{m}(3)-bbox_current{m}(1) , bbox_current{m}(4)-bbox_current{m}(2)];
   wx = deform_param_patch{m}(1);
   wy = deform_param_patch{m}(2);
   
   if quadratic_flg
       [dx,dy,dx2,dy2] = quadratic_deform(detected_bbox,anchor_bbox,root_bbox);
       sp_scores{m} = sum([wx wy wx.*wx wy.*wy].*[dx dy dx2 dy2]);%Quadratic cost
   else
       intersect = rectint(detected_bbox,anchor_bbox);
       union = detected_bbox(3)*detected_bbox(4) + anchor_bbox(3)*anchor_bbox(4) - intersect;
       sp_scores{m} = intersect / union;%intersection over union %%%%%UUUWWWW
   end
