function [bs, block_sums, block_counts] = ...
  gdetect_write(pyra, model, bs, trees, from_pos, ...
                dataid, max_size, max_num, block_sums, block_counts)
% Write detections from gdetect.m to the feature vector cache.
%   [bs] = gdetect_write(pyra, model, bs, trees, from_pos, ...
...
if ~exist('block_sums', 'var') || ~exist('block_counts', 'var')
  block_sums   = [];
  block_counts = [];
end

return_feat = (nargout > 1);

count = 0;
if ~isempty(bs)
  [count, block_sums, block_counts] = ...
    write_features(pyra, model, trees, from_pos, dataid, max_size, max_num, ...
                   block_sums, block_counts, return_feat);
  % truncate boxes
  bs(count+1:end,:) = [];
end
...
% ------------------------------------------------------------------------
function [count, block_sums, block_counts] = ...
  write_features(pyra, model, trees, from_pos, dataid, max_size, max_num, ...
                 block_sums, block_counts, return_feat)
% ------------------------------------------------------------------------
...
if isempty(block_sums) || isempty(block_counts)
...
for d = 1:min(max_num, length(trees))
  ...
  if return_feat
    weight = 1;
    %weight = ex.score;
    for b = 1:length(ex.blocks)
      if ~isempty(ex.blocks(b).f)
        block_sums{b} = block_sums{b} + ex.blocks(b).f * weight;
        block_counts(b) = block_counts(b) + weight;
      end
    end
  end
