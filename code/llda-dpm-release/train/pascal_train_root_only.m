...
save_file = [cachedir cls '_lrsplit1_llda'];
try
  load(save_file);
catch
  seed_rand();
  for i = 1:n
    ...
    models{i} = train_lda(models{i}, spos{i}(inds), true, 1, fg_overlap, ...
                          ['lrsplit1_llda_' num2str(i)]);
  end
...
save_file = [cachedir cls '_lrsplit2_llda'];
try
  load(save_file);
catch
  seed_rand();
  for i = 1:n
    ...
    models{i} = train_lda(models{i}, spos{i}, false, 4, fg_overlap, ...
                          ['lrsplit2_llda_' num2str(i)]);
  end
  save(save_file, 'models');
end
...
save_file = [cachedir cls '_mix_llda'];
try
  load(save_file);
catch
  seed_rand();
  ...
  model = train_lda(model, impos, false, 7, fg_overlap, 'mix_llda');
  save(save_file, 'model');
end

% Set to false if you want a pure latent LDA classifier
use_hard_negatives = true;
if use_hard_negatives
  save_file = [cachedir cls '_mix_hard_neg'];
  try
    load(save_file);
  catch
    % Finish training by data mining on all of the negative images
    model = train(model, impos, neg_large, false, false, 1, 5, ...
                  max_num_examples, fg_overlap, num_fp, false, 'mix_hard_neg');
    save(save_file, 'model');
  end
end
