...
hyp = init_hyp_many(cached_scores, Nreg, model, Ntodo);
...
for pi = 1:model.num_parts
   ...
   for level = 1:length(scales)
      ...
      for i_x_sh = 1:1 % Not supported! length(model.shift)
         x_shift = model.shift(i_x_sh);
         for i_y_sh = 1:1 % Not supported! length(model.shift)
            y_shift = model.shift(i_y_sh);
            for i_rot = 1:1 % Not Supported! length(model.rotation)
...
%                  ys = ys0(1:Sy, 1:Sx);
      ...
      % Bookkeeping for getting top detections...
      all_boxes0{level} = boxes;
      all_scores0{level} = score_map(:);
      all_locs0{level} = [xs(:), ys(:), repmat(level, numel(xs), 1), score_ind(:), repmat([1 1 1], numel(xs), 1)] ;
   end % level
   ...
   all_boxes = cat(1, all_boxes0{:});
   all_scores = cat(1, all_scores0{:});   
   all_locs = cat(1, all_locs0{:});
   % Now do NMS
 
   [best_score_all best_inds] = get_best_parts(regions, all_boxes, all_scores, const(1), const(2), const(3), const(4), const(5), const(6), Ntodo, 0.3);
   
   best_boxes = ones(size(regions,1), 4, Ntodo);
   Nloc = size(all_locs,2);
   best_locs = ones(size(regions,1), Nloc, Ntodo);
   best_scores2 = -inf(size(regions,1), Ntodo);

   for j = 1:Ntodo 
      ok = best_inds(:, j)>0;
      ok_inds = best_inds(ok, j);
      best_boxes(ok, :, j) = all_boxes(ok_inds, :);
      best_locs(ok, :, j) = all_locs(ok_inds, :);
      best_scores2(ok, j) = all_scores(ok_inds, :);
   end

   hyp = update_hyp(hyp, best_scores2, best_locs, best_boxes, i);    
end
...
function hyp = init_hyp_many(init_score, Nreg, model, Ntodo)
...
for r = 1:length(hyp)
   hyp(r).region = r;
   hyp(r).computed = zeros(toadd, 1, Ntodo);
   hyp(r).score = zeros(toadd, Ntodo);
   hyp(r).loc = zeros(toadd, 3, Ntodo);
   hyp(r).bbox = zeros(toadd,4, Ntodo);
