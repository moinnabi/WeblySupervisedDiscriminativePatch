function [all_scores detection_loc flipped] = part_inference(input, part_model, regions)
...
padx = 6;%ceil(part_model.hg_size(2)/2+1);
pady = 6;%ceil(part_model.hg_size(1)/2+1);
...
for m = 1:length(part_model)
   best_scores = -inf(size(regions{m},1), 1);
   best_loc = ones(size(regions{m},1), 4); % [x y s f];
...
%          showboxes(input,top_boxes)
         ...
         [best_score pos] = get_best_part(regions{m}, boxes, scores, const(1), const(2), const(3), const(4), const(5), const(6)); %0, 1, 0.8, 1, 0.01, 1);
