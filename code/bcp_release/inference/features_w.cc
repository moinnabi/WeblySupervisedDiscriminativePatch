...
mxArray *process_weighted(const mxArray *mximage, const mxArray *mxweights, const mxArray *mxsbin) {
  double *im = (double *)mxGetPr(mximage);
  double *weight_im = (double *)mxGetPr(mxweights);

  const int *dims = mxGetDimensions(mximage);
  const int *w_dims = mxGetDimensions(mxweights);
  if (mxGetNumberOfDimensions(mximage) != 3 ||
      dims[2] != 3 ||
      mxGetClassID(mximage) != mxDOUBLE_CLASS ||
      mxGetClassID(mxweights) != mxDOUBLE_CLASS ||
      w_dims[0]!=dims[0] || w_dims[1]!=dims[1])
  ...
  for (int x = 1; x < visible[1]-1; x++) {
    for (int y = 1; y < visible[0]-1; y++) {
      ...
      double w = *(weight_im + min(x, dims[1]-2)*dims[0] + min(y, dims[0]-2));
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
  ...
  else if(nrhs == 3) // do weighted work
     plhs[0] = process_weighted(prhs[0], prhs[1], prhs[2]);
}
