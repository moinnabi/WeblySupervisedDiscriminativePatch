...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
   ...
   for(int i=0; i<NA; i++) {
      ...
      double bin_h = (A[i+3*NA]-A[i+NA])/(spat_h);
      double bin_w = (A[i+2*NA]-A[i])/(spat_w);
      double y0 = A[i+NA];
      double x0 = A[i];
...
//      printf("\n");
      ...
      for(int j=0; j<NB && !skip; j++) {
         //printf("%d %d\n", i, j);
         if(1 ||(B_score[j]+max_spat)>max_score) { // Only consider a detection if its upper bound exceeds current max score
         //if(B_score[j]>0) {
            // Check the overlap
            ...
            double ycenter = 1./2*(B[j+3*NB]+B[j+NB]);
            double xcenter = 1./2*(B[j+2*NB]+B[j]);
            double scale = Barea[j];

            y = (int)min(spat_h-1, max(0, floor((ycenter-y0)/bin_h)));
            x = (int)min(spat_w-1, max(0, floor((xcenter-x0)/bin_w)));
