...
mxArray *resize(const mxArray *mxsrc, const mxArray *mxscale) {
  ...
  ddims[1] = (int)round(sdims[1]*scale);
   ...
   safe_sdims[0] = sdims[0];
   safe_sdims[1] = sdims[1];

   if(mxGetNumberOfDimensions(mxsrc)>=3) {
     ddims[2] = sdims[2];
     safe_sdims[2] = sdims[2];
   } else {
     ddims[2] = 1;
     safe_sdims[2] = 1;
   }
  ...
  double *tmp = (double *)mxCalloc(ddims[0]*safe_sdims[1]*safe_sdims[2], sizeof(double));
  resize1dtran(src, safe_sdims[0], tmp, ddims[0], safe_sdims[1], safe_sdims[2]);
  resize1dtran(tmp, safe_sdims[1], dst, ddims[1], ddims[0], safe_sdims[2]);
  mxFree(tmp);
