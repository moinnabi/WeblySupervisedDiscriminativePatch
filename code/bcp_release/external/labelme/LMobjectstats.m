...
if nargout == 0
    ...
    figure
    subplot(221)
    barh(frequency(ndx(1:40)))
    set(gca, 'YTick', 1:40)
    set(gca, 'YtickLabel', objectnames(ndx(1:40)))
    axis([0 max(frequency)+5 0 40+1])
    grid on
    ...
    barh(counts(ndx(1:20)))
    set(gca, 'YTick', 1:20)
    set(gca, 'YtickLabel', scenenames(1:20))
    axis([0 max(frequency)+5 0 20+1])
    grid on
    axis('tight')
