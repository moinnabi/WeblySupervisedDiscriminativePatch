        ...
        if(isstr(objectlist))
            ...
            for i = 1:length(j)-1
                ...
                for n = 1:length(jc)
                    ...
                    %maskpol(:,:,ni) = logical(inpolygon(x, y, X, Y));
                    classpol(ni) = i;
            ...
            Nobjects = length(objectlist);%length(annotation.object);
            %[x,y] = meshgrid(1:ncols,1:nrows);
 
            mask = zeros([nrows, ncols, Nobjects]);
            for i = 1:Nobjects
                class{i} = annotation.object(objectlist(i)).name; % get object name
                [X,Y] = getLMpolygon(annotation.object(objectlist(i)).polygon);
