...
%  actually replace with <tag el1='asdf'/></tag>
%  then <tag><el1>asdf</el1></tag>

xml = strrep(xml, '< ', '<');
xml = strrep(xml, ' >', '>');

je = regexp(xml, '[^/]>');
% I apologize for the butchery of this code
while length(je)>0
    jo = strfind(xml, '<');
    l = jo(max(find(jo < je(1))));
    tok_init = xml(l+1:je(1));
    je_min = je(1);

    tok_end = strfind(tok_init, ' ');
    if(~isempty(tok_end))
       tok = tok_init(1:tok_end(1)-1);

       attributes = tok_init(tok_end+1:end);
       attribute_match = regexp(attributes, '(?<a>[^= ]+)=[''"]?(?<b>[^=''"]+)[''"]?(\s*)', 'names');
       attribute_xml = [];
       for i = 1:length(attribute_match)
         attribute_xml= [attribute_xml sprintf('<%s>%s</%s>', attribute_match(i).a, attribute_match(i).b, attribute_match(i).a)];
       end

       xml = strrep(xml, sprintf('<%s %s>', char(tok), attributes), sprintf('<%s>%s', char(tok), attribute_xml));
      % je = strfind(xml, '/>');
      je = regexp(xml, '[^/]>');
   end
...
while length(je)>0
    jo = strfind(xml, '<');
    l = jo(max(find(jo < je(1))));
    tok_init = xml(l+1:je(1)-1);
    tok_end = strfind(tok_init, ' ');
    tok = tok_init(1:tok_end-1);
...
%    attribute_match = regexp(attributes, '(?<a>[^= ]+)=(?<b>[^= ]+)(\s*)', 'names')
    ...
    xml = strrep(xml, sprintf('<%s %s/>', char(tok), attributes), sprintf('<%s>%s</%s>', char(tok), attribute_xml, char(tok)));
    je = strfind(xml, '/>');
end
