...
for opt=1:optim
    ...
    for k=1:iter        
        ...
        for j=1:Nc
            ...
            %h(:,j)=p(j)*exp(-0.5*(dym+dxm))/sqrt(det(Cy(:,:,j)))/(2*pi)^(Dy/2+Nf/2)*sqrt(det(iXa))/(2*pi)^(Nf/2);
            h(:,j)=log(p(j))-0.5*(dym+dxm)-log(sqrt(det(Cy(:,:,j))))-log((2*pi)^(Dy/2+Nf/2))+log(sqrt(det(iXa)))-log((2*pi)^(Nf/2));
            %ss=ss+h(:,j);
        end
        toc

        h = h - repmat(max(h,[],2), [1 Nc]);
        h = exp(h);
