function [recall, precision, Dd, threholds, score, correct] = LMrecallPrecision(Dgt, Dd)
...
for i = 1:NimagesGt
    ...
    [xt,yt] = LMobjectpolygon(Dgt(i).annotation);
    Bt = LMobjectboundingbox(Dgt(i).annotation);
    Ngt = size(Bt,1);

    % Get polygons detection
    [xd,yd] = LMobjectpolygon(Dd(i).annotation);
    Bd = LMobjectboundingbox(Dd(i).annotation);
    Nd = size(Bd,1);
...
Ntargets = sum(LMcountobject(Dgt));
