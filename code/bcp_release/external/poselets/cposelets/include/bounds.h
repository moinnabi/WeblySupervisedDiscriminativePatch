...
inline int intround(float x ) { return static_cast<int>(x + (x < 0.0f ? -0.5f : 0.5f)); }
inline int intround(double x) { return static_cast<int>(x + (x < 0.0  ? -0.5  : 0.5 )); }

inline boost::gil::point2<int> intround(const boost::gil::point2<float >& p)  { return boost::gil::point2<int>(intround(p.x),intround(p.y)); }
inline boost::gil::point2<int> intround(const boost::gil::point2<double>& p)  { return boost::gil::point2<int>(intround(p.x),intround(p.y)); }

////////////////////////////////////////////////////////////////////////////////////////
/// CLASS bounds
///
/// A 2D bounding box represented as a pair of top left (_min) and bottom right (_max) points
/// The coordinates of _max must be greater or equal to those in _min.
///
////////////////////////////////////////////////////////////////////////////////////////
...
struct bounds {
    ...
    bounds() : _min(std::numeric_limits<T>::max(), std::numeric_limits<T>::max()), _max(std::numeric_limits<T>::min(), std::numeric_limits<T>::min()) {}
    bounds(const point_type& minPt, const point_type& maxPt,bool) : _min(minPt), _max(maxPt)                   {}
    bounds(const T& xMin, const T& yMin, const T& xSize, const T& ySize)    : _min(xMin,yMin), _max(xMin+xSize,yMin+ySize) {}
    bounds(const T& xMin, const T& yMin, const T& xMax, const T& yMax,bool)    : _min(xMin,yMin), _max(xMax,yMax)               {}
    bounds(const point_type& minPt, const point_type& sz)        : _min(minPt), _max(minPt+sz)                   {}
    template <typename T1> explicit bounds(const bounds<T1>& p) : _min(p._min), _max(p._max) {}

    bounds(const bounds& b)                                    : _min(b._min), _max(b._max) {}
    const bounds&    operator=(const bounds& b)                { _min=b._min; _max=b._max; return *this; }
	...
	const T& operator[](std::size_t i)          const   { return reinterpret_cast<const T*>(this)[i]; }// unsafe
          T& operator[](std::size_t i)                  { return reinterpret_cast<      T*>(this)[i]; }// unsafe
    
    point_type _min,_max;
};

template <typename T> std::ostream& operator<<(std::ostream& os, const bounds<T>& b) { return os << b._min<<" "<<b._max<<" ";; }
template <typename T> std::istream& operator>>(std::istream& is,       bounds<T>& b) { return is >> b._min >> b._max; }

template <typename T> inline
bool operator==(const bounds<T>& b1, const bounds<T>& b2) { return (b1._min==b2._min && b1._max==b2._max); }

template <typename T> inline
bool operator!=(const bounds<T>& b1, const bounds<T>& b2) { return  !(b1==b2); }

template <typename T> inline
bounds<T> operator+(const bounds<T>& b, const boost::gil::point2<T>& pt) { return bounds<T>(b._min+pt,b.size()); }
...
bool bounds<T>::intersects(const bounds& b) const {
    return !(_min.x>b._max.x || _min.y>b._max.y || _max.x<b._min.x || _max.y<b._min.y);
}
...
template <typename T>
inline bounds<T> union_bounds(const bounds<T>& b1,const bounds<T>& b2) { //unfortunately union is a C++ keyword
...
template <typename T> inline T area(const bounds<T>& b) { return b.width()*b.height(); }
...
inline void interpolate_bounds(const Bounds2& b2, Scalar w2, Bounds1& accum, Scalar w1=1) {
...
Bounds get_interpolated_bounds(const Bounds& b1, Scalar w1, const Bounds& b2, Scalar w2) {
	return Bounds(get_interpolated_point(b1._min,w1,b2._min,w2), get_interpolated_point(b1._max,w1,b2._max,w2),true);
}

template <typename MetaView>
inline bounds<int> view_bounds(const MetaView& v) { return bounds<int>(0,0,(int)v.width(),(int)v.height()); }
