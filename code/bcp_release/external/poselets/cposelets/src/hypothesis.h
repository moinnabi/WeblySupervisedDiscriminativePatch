...
public:
	...
	hypothesis(const hypothesis& h) : _coords_sum(h._coords_sum), _coords_sum2(h._coords_sum2), _w_sum(h._w_sum), _kp_distribs(h._kp_distribs), _rect(h._rect) {}
	...
	const float_point& kp_mean(size_t kp_id) const { return _kp_distribs[kp_id]._mu; }
	const float_point& kp_sigma(size_t kp_id) const { return _kp_distribs[kp_id]._sigma; }
private:
