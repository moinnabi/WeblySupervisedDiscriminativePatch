...
poselet::poselet(rapidxml::xml_node<>* node, int num_poselets, int num_keypoints) : _hypothesis(node, num_keypoints) {
	...
	}
	{
		...
		float_point dims;
		str >> _obj_bounds_var._min.x >> _obj_bounds_var._min.y >> dims.x >> dims.y;
		_obj_bounds_var._max =  _obj_bounds_var._min + dims;
	}

	if (node->first_node("bigq_weights")!=NULL)
		_bigq_classifier = linear_classifier<float>(node->first_node("bigq_weights"));
	...
	int bin_size=config::get().bin_size();
	assert(int(_classifier._weights.size())==1 + bin_size*cell_dims.x*cell_dims.y);
