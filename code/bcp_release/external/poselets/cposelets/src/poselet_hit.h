...
//	hit(double score, const float_point& mn, const float_point& dims) : _score(score), _bounds(mn,dims) {}
	hit(double score, const float_bounds& bounds) : _score(score), _bounds(bounds) {}
	...
	friend bool operator==(const hit& a, const hit& b) { return a._bounds==b._bounds && a._score==b._score; }
	...
	void draw(const View& v, const Color& c, int thickness=1) const {
		...
		render_text(v, cast_point<int_point>(_bounds._min)+int_point(thickness,thickness), str(boost::format("%4.2f") % _score), c, 1, 1);
	}
private:
	double _score;
...
public:
	poselet_hit() {}
	poselet_hit(int pid, double score, const float_bounds& bounds) : hit(score,bounds), _poselet_id(pid) {}
//	poselet_hit(int pid, double score, const float_point& mn, const float_point& dims) : hit(score,mn,dims), _poselet_id(pid) {}
	...
	friend bool operator==(const poselet_hit& a, const poselet_hit& b) { return static_cast<const hit&>(a)==static_cast<const hit&>(b) && a._poselet_id==b._poselet_id; }
