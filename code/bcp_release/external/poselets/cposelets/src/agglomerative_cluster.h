...
void agglomerative_cluster(ElementInIt elementsIt, size_t num_elements, const DistFn& dist_fn, const MergeFn& merge_fn, DistType threshold, ElementOutIt result) {
	using namespace std;
	...
	for (size_t i=0; i<num_elements-1; ++i)
		for (size_t j=i+1; j<num_elements; ++j)
			distances.insert(make_pair(dist_fn(clusters[i],clusters[j]), make_pair(i,j)));
	...
	while (!distances.empty()) {
		...
		clusters.push_back(merge_fn(clusters[closest_pair.first],clusters[closest_pair.second]));
		valid.push_back(true);
		valid[closest_pair.first]=false;
		valid[closest_pair.second]=false;
		for (size_t i=0; i<clusters.size()-1; ++i)
			if (valid[i])
				distances.insert(make_pair(dist_fn(clusters.back(),clusters[i]), make_pair(i,clusters.size()-1)));
	}
