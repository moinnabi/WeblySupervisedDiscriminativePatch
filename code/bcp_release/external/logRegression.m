% function x = logRegression(a, y, w, ridge, param)
%
% Logistic regression.  Design matrix A, targets Y, optional
% instance
% weights W, optional ridge term RIDGE, optional parameters object
% PARAM.
%
% W is a vectorlength equal to the number of regressors, or
% a scalar (the latter being s with length equal to the number of
% training examples; RIDGE
% can be either a vector with ynonymous to a vector with all
% entries the
% same).
%
% PARAM has fields PARAM.MAXITER (an iteration limit),
% PARAM.VERBOSE
% (whether to print diagnostic information), PARAM.EPSILON (used to
% test
% convergence), and PARAM.MAXPRINT (how many regression
% coefficients to
% print if VERBOSE==1).
%
% Model is 
