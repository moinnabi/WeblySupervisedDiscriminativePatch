function [rec,prec,ap] = VOCevaldet(VOCopts,id,cls,draw, dup_penalty, varargin)


if(~exist('dup_penalty','var'))
   dup_penalty = 1;
end
% load test set
...
if exist(cp,'file')
    ...
    [gtids,t]=textread(sprintf(VOCopts.imgsetpath,VOCopts.testset),['%s ' ...
    '%d']);
    tic;
...
for d=1:nd
    ...
    if ovmax>=VOCopts.minoverlap
		...
		gt(i).det(jmax)=true;
                ...
                fp(d)=dup_penalty;            % false positive (multiple detection)
            end
        end
    else
