function [pos_prec chosen aps] = choose_candidate_amp_subset(model, ...
                                                  D, cached_scores, ...
                                                  candidates, numpos, trial)
...
for i = 1:length(candidates)
   %fprintf('%d/%d\n', i, num_parts);
   [aps(i) pos_prec{i}] = test_part_detections_wrapper(D, cached_gt, ...
                                                     cls, candidates(i), ...
                                                     numpos, trial);
   fprintf('%f\n', aps(i));
end
...
function [aps pos_prec] = test_part_detections_wrapper(D, cached_scores, ...
                                                  cls, candidate, ...
                                                  numpos, trial);
   results = test_candidate_detections_subset(D, cached_scores, cls, ...
                                              candidate, [], [], ...
                                              'auto_exemplars', ...
                                              numpos, trial);
   ...
   if(~isempty(results{1}))
      ...
      [recall prec aps pos_prec] = test_part_detections_D(cls, ...
                                                        D, cached_scores, ...
                                                        full_results);
   else
      aps = 0;
