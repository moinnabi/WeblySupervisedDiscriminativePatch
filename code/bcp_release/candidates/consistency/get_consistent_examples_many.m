function cached_scores = get_training_examples(model, D, cached_scores)
...
Dneg = D(neg_inds);
cached_neg = cached_scores(neg_inds);

keyboard
parfor i = 1:length(Dpos)
   fprintf('%d/%d\n', i, length(Dpos));
   im = imread(fullfile(im_dir, Dpos(i).annotation.filename));
   [boxes{i} flipped{i}] = inference_part(im, model, NMS_IOU);
end

parfor i = 1:length(Dneg)
   fprintf('%d/%d\n', i, length(Dneg));
   im = imread(fullfile(im_dir, Dneg(i).annotation.filename));
   [boxes_neg{i} flipped_neg{i}] = inference_part(im, model, NMS_IOU);
end
...
exemp_box = feat_all(best_obj, 1:4);
ok = find(bbox_overlap_mex(feat_all(:, 1:4), exemp_box)>=0.8 & pos_probs>=0.5); % These thresholds are arbitrary
