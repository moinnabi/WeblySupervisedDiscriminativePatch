...
if length(m.model.mask(:)) ~= numel(m.model.w)
  m.model.mask = repmat(m.model.mask, [1 1 QBfeatures]);
...
if (~isfield(m.model, 'prior') || isempty(m.model.prior))
  %old method
...
%  svm_model = libsvmtrain(supery, newx',sprintf(['-s 0 -t 0 -c' ...
%                    ' %f -w1 %.9f -q'], mining_params.SVMC, wpos));
  m.model.prior = m.model.x;
  mining_params.SVMG = 0;
  
  svm_weights = svm_prior(supery, superx, mining_params.SVMC, wpos, m.model.prior, mining_params.SVMG);

   wex = svm_weights(1:end-1);
   b = -svm_weights(end);
