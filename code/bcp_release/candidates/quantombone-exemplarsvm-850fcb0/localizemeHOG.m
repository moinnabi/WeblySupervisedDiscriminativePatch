...
templates = zeros(S(1),S(2),QBfeatures,length(models));
templates_x = zeros(S(1),S(2),QBfeatures,length(models));
template_masks = zeros(S(1),S(2),QBfeatures,length(models));

for i = 1:length(models)
  t = zeros(S(1),S(2),QBfeatures);
  t(1:models{i}.model.hg_size(1),1:models{i}.model.hg_size(2),:) = ...
      models{i}.model.w;

  templates(:,:,:,i) = t;
  template_masks(:,:,:,i) = repmat(double(sum(t.^2,3)>0),[1 1 QBfeatures]);

  if (length(localizeparams.nnmode) > 0) || ...
        (isfield(localizeparams,'wtype') && ...
         strcmp(localizeparams.wtype,'dfun')==1)
    x = zeros(S(1),S(2),QBfeatures);
...
X = zeros(S(1)*S(2)*QBfeatures,sumN);
...
for i = 1:length(t.hog)
  ...
  curf = reshape(t.hog{i},[],QBfeatures);
