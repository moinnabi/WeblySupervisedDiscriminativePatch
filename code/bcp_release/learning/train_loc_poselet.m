...
for outer_iter = 1:Nouter
   if(outer_iter==-1)  % Select top K examples using distance measure 
      [pos_feats0 pos_im_ind0 dist0] = get_poselet_examples_app(model, Dpos, cached_pos); %collect_training_ex(model, Dpos, cached_pos, 1);
      ...
      % Positive scores
      if(mod(outer_iter,2)==1) % Update the set of examples to use
         pos_scores = w(1:end-1)'*cat(2, pos_feats0{:}) + w(end);
         fprintf('Updating scores\n');
      ...
      pos_set_ok = pos_probs>=prob_thresh;
      else
         pos_set_ok = pos_scores>=-1;
      end

      else
         fprintf('Not updating scores to remain compatible with consistency training\n');
      end

      score_thresh = min(pos_scores(pos_set_ok));
   
      fprintf('==================Thresholding examples at %d==========\n', score_thresh);
      pos_feats1 = {};
      pos_im_ind1 = {};
      for p = 1:length(pos_feats0)
         if(~isempty(pos_feats0{p}))
