...
obj_box = kp.bbox;
procrust_scale = sqrt(sum((obj_box([3 4]) - obj_box([1 2])).^2)); %eps + 1/length(gt_ind)*sqrt(sum((kp_gt(:, 1) - mean(kp_gt(:,1))).^2) + sum((kp_gt(:, 2) - mean(kp_gt(:,2))).^2));
...
[x y] = meshgrid(1:size(f_real{1},2)+padx+2, 1:size(f_real{1},1)+pady+2);
...
for i = 1:length(f_real)
   score_map = fconv(f_real{i}, {model.filter}, 1, 1);
   scores = score_map{1};
   
   score_map_flip = fconv(f_real{i}, {flipfeat(model.filter)}, 1, 1);
   scores_flip = score_map_flip{1};
   
   xs = x(1:size(scores,1), 1:size(scores,2));
   ys = y(1:size(scores,1), 1:size(scores,2));
   ...
   pred_err = sqrt(bsxfun(@minus, pred_kpX, kp_gt(gt_kp_todo, 1)').^2 + bsxfun(@minus, pred_kpY, kp_gt(gt_kp_todo, 2)').^2);
   pred_err_sc = pred_err/procrust_scale;
   pred_err_count = sum(pred_err_sc<=0.1,2);
   overall_score = scores(:) + pred_err_count*100;
   ...
   pred_err_flip = sqrt(bsxfun(@minus, pred_kpX_flip, kp_gt(gt_kp_todo_flip, 1)').^2 + bsxfun(@minus, pred_kpY_flip, kp_gt(gt_kp_todo_flip, 2)').^2);
   pred_err_sc_flip = pred_err_flip/procrust_scale;
   pred_err_count_flip = sum(pred_err_sc_flip<=0.1, 2);
   overall_score_flip = scores_flip(:) + pred_err_count_flip*100;
   
   [min_dist_t ind] = min(-overall_score);
   ...
   [min_dist_t ind] = min(-overall_score_flip);
