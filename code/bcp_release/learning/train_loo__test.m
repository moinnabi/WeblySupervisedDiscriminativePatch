function [model neg_feats w_loo w_orig all_models] = train_boosted(model, D, cached_scores, num_iter, Nouter, rate, C, neg_feats)
...
for outer_iter = 1:Nouter
   ...
   for inner_iter = 1:num_iter%min(num_iter, ceil(N/blocksize))
      ...
      if(1 || inner_iter==1)
         ...
         [w deltas alphas obj] = subset_svm(feats, labels, C, ceil(sum(labels==1)*rate), delta0);
         ...
         [w deltas alphas] = subset_svm(feats, labels, C, ceil(sum(labels==1)*rate), new_deltas);
         delta_pos = deltas(1:size(pos_feats,2));
      end
      clear feats; 
...
if(do_loo)
   ...
   for i = 1:length(pos_im_todo)
      ...
      if(fast_subset) % Just train on precomputed subset
         ...
         if(numel(C)==1)
            w_loo0{i} = svm_dual_mex(labels(delta1==1), feats(:, delta1==1), C, [], alpha0)'; % Initialize using previous alpha
         else
            warning('This isn''t implemented!\n');
            [wt b] = fast_svm(labels(delta1==1), feats(:, delta1==1), C(1), C(2)/C(1));
            w_loo0{i} = [wt(:); b];
         end
      else
         w_loo0{i} = subset_svm(feats, labels, C, ceil(sum(labels==1)*rate));
      end
