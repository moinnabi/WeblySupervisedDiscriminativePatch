...
//   oact.sa_handler(sig);
   mexErrMsgTxt("Caught SIGINT, terminating");
}
#define IN_CACHE 2
#define WAIT 20
...
void optimize_dual(double *labels, double *feat, double C, double *reg, int N, int Nw, double *w, double *alphas)
{
   ...
   while(!terminate) {
      ...
      for(pi = 0; pi<N; pi++) {
         ...
         eta_r = eta1/etas[i];
         alpha_new = min(max(alphas[i] - eta_r, 0), C);
...
//        0       1          2       3       4    5  6   7
   ...
   if(nrhs>=5 && mxGetNumberOfElements(prhs[4])==N) {
      printf("Initialzing alphas 2\n");
      double *alpha0 = mxGetPr(prhs[4]);
      for(int i=0; i<N; i++)
         alphas[i] = max(min(alpha0[i], C), 0);  // Reproject just to be safe

      // Initialize wt as well
      for(int i=0; i<N; i++) {
         for(int j=0; j<Nw; ++j)
            wt[j] += alphas[i]*labels[i]*feat[j+Nw*i]/reg[j];

         wt[Nw] += alphas[i]*labels[i]/reg[Nw];
      }
      printf("done!\n");
   }
...
//   optimize_dual(alpha_out, Nw, C, prhs[0], prhs[1], prhs[2], mxGetPr(prhs[3]), Q, N, mxGetPr(prhs[6]), wt);
//   optimize_dual(mxGetPr(prhs[0]), mxGetPr(prhs[1]), C, reg, N, Nw, wt, alphas);
