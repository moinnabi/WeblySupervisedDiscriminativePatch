function [model all_models] = train_boosted(model, D, cached_scores, num_iter, Nouter, rate)
...
for outer_iter = 1:Nouter
   ...
   for inner_iter = 1:min(num_iter, ceil(N/blocksize))
      ...
      [w deltas alphas] = subset_svm(feats, labels, 1e-2, ceil(sum(labels==1)*rate));
   
      if(inner_iter==1000)
          keyboard
          
      for i = 1:174 
        [wi{i} deltasi{i} alphasi{i}] = subset_svm(feats(:,[1:i-1, i+1:end]), labels([1:i-1 i+1:end]), 1e-2, ceil((sum(labels==1)-1)*rate));
      end
