function [ds_moin_rescored,scores_all_rescored] = moin_rescoring_moin(model_selected,relpos_patch_normal_selected,voc_test,ds_moin,w_sel_moin,suffix_moin)
% function for rescoring bounding box of santosh by parts defined by moin

ds_moin_rescored = ds_moin;
%scores_all_rescored = [];
...
try
    ...
    load(['data/',suffix_moin,'.mat'],'scores_all_rescored');
catch
    ...
    save(['data/',suffix_moin,'.mat'],'scores_all_rescored');
end
...
for ind =1:ds_ind-1
    ...
    i = ds_adrs(ind,1);
    j = ds_adrs(ind,2);
    ds_moin_rescored{i}(j,5) = score_all(ind);
end