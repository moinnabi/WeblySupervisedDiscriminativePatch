function pascal_eval_calibHO_wsup(cls, baseobjname, cachedir, testset, year, suffix)
...
diary([cachedir '/diaryoutput_eval_calibHO_' testset '.txt']);
disp(['pascal_eval_calibHO_wsup(''' cls ''',''' baseobjname ''',''' cachedir ''',''' testset ''',''' year ''',''' suffix ''')' ]);
...
%{
% %5Dec12: I now updated the .txt files to be val2.txt and test1.txt; so no
longer need to do this
hoinds = textread(sprintf(VOCopts.imgsetpath, 'hoinds'), '%d');

load([cachedir cls '_boxes_test_' suffix], 'ds');
ds = ds(hoinds);
...
if do_eval
    if str2num(year) == 2006
        ...
        tic;
        [recall, prec, ap, tp, fp, ov] = VOCevaldet_calibHO_wsup(VOCopts, 'comp5', cls, baseobjname, false);
    end
end
...
% save results
save([cachedir '/' cls '_calibParamsHO_' testset '.mat'], 'sigAB', 'confidence', 'ids', 'labels', 'ovp');
if DISP, saveas(gcf, [cachedir '/display/sigABplotHO_' testset '.jpg']); end
fprintf('AP = %.4f \n', ap);
