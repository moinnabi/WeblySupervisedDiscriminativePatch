function [ds_all, bs_all, X] = context_dataWithLoc_wsup(cachedir, dataset, year, cls_inds, phrasenames)

% copied from context_data_wsup.m; includes location info for max
% detections

% this script is incomplete (i didn't add the loc info)
...
try
    load([cachedir 'context_dataWithLoc_' dataset '_' year]);
catch
    ...
    save([cachedir 'context_dataWithLoc_' dataset '_' year], 'X', ...
        'ds_all', 'bs_all');
    fprintf('done!\n');
end
