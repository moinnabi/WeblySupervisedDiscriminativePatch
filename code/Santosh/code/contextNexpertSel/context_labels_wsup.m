function [labels, olapval] = context_labels_wsup(cachedir, cls, ds, train_set, train_year)

% copied from context_labels; no modifications yet
...
try
    ...
    [gt, npos] = get_ground_truth(cachedir, cls, train_set, train_year);
    [gtids, t] = textread(sprintf(VOCopts.imgsetpath, train_set),'%s %d');
    if length(gtids) ~= size(ds,1), disp('dimension mismatch'); keyboard; end
    ...
    for d=1:nd
        ...
        if ovmax>=VOCopts.minoverlap
            if ~gt(i).diff(jmax)
                ...
                labels{i}(idx(d)) = 0;  % setting it to zero as unsup case i want to avoid those samples (22Jan13)
            end
        else
    ...
    save([cachedir cls '_context_labels_' train_set '_' train_year], 'labels', 'olapval');
    fprintf('done!\n');
end
