...
% choose unique threshold scores
...
mask = repmat(y, 1, n) >= repmat(s, length(t), 1);
tp   = sum(repmat(t,   1, n).*mask)'/sum(t);
fp   = sum(repmat(1-t, 1, n).*mask)'/sum(1-t);
