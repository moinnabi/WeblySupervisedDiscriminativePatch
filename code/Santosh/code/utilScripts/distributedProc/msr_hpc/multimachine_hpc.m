function multimachine_hpc(singleMachFunc, numClasses, resdir, NUM_MACHS, PROC_NAME, NUM_PROCS, NUM_GB) 
...
init_matlab_cmdstr = '';
cmd = sprintf(['cd %s; %s; exit '], pwd, singleMachFunc);
...
if ~isempty(resdir)
    ...
    while ~all_done
        pause(10);
        done_files = dir([resdir doneDirName '/*.done']);
        lock_files = dir([resdir doneDirName '/*.lock']);
        err_files = mydir([resdir doneDirName '/*.error']);
