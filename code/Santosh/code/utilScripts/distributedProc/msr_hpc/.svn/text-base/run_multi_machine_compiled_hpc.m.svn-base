function run_multi_machine_compiled_hpc(cmd_str, machineInfo, runMode)
%
...
if strcmp(runMode, 'newCl')
    ...
    for i_job = 1:num_jobs
       ...
       [blah pbsjobid] = system(['job submit /scheduler:' clusterName ' /jobname:ls /numcores:1 dir']);
        jobidnum = str2num(pbsjobid(29:end-1))+1; 
        
        OUTPUT_FILER=[logdir filesep clusterName '_' num2str(jobidnum) '.warp.output'];
        ERROR_FILER=[logdir filesep clusterName '_' num2str(jobidnum) '.warp.error'];
        finalcmd = [cmd ' > ' OUTPUT_FILER];
                            
        %disp('here'); keyboard;
        system(['job submit /scheduler:' clusterName ' /jobname:' ID_str ' /stdout:' OUTPUT_FILER ...
