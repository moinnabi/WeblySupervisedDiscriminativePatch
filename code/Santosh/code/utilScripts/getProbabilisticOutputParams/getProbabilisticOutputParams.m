function [A, B, err, dtype] = getProbabilisticOutputParams(conf, labels)
...
AB = fminsearch(@(AB) logisticError(AB, conf, labels), [0 log((sum(labels==0)+1)/(sum(labels==1)+1))], []);%, optimset('MaxFunEvals', 1000000, 'MaxIter', 1000000));
A2 = AB(1); B2 = AB(2);
