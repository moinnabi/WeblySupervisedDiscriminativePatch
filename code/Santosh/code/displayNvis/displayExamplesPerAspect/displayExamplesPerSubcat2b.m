function displayExamplesPerSubcat2b(objname, outdir, numComps, VOCyear, traindatatype)

try    
disp(['displayExamplesPerSubcat2b(''' objname ''',''' outdir ''',' num2str(numComps) ',''' VOCyear ''')' ]);
...
try load([outdir '/' objname '_' modeltype '.mat'], 'inds_lrs1',...
    ...
    if ~isfield(model, 'overlap'), model.overlap = OVLAP; end
...
try 
    ...
    if ~isfield(model, 'overlap'), model.overlap = OVLAP; end
    if isfield(model, 'sfx'), pos = enlargeWithContext(pos, model.sfx, model.sfy); end
    disp(' lrsplit2_1: getting subcategory membership info');
...
try 
    ...
    if ~isfield(model, 'overlap'), model.overlap = OVLAP; end
    if isfield(model, 'sfx'), pos = enlargeWithContext(pos, model.sfx, model.sfy); end
    disp(' LRSPLIT2_2: getting subcategory membership info');
...
try 
    ...
    if ~isfield(model, 'overlap'), model.overlap = OVLAP; end
    if isfield(model, 'sfx'), pos = enlargeWithContext(pos, model.sfx, model.sfy); end
    disp(' LRSPLIT2_3: getting subcategory membership info');
...
try load([outdir '/' objname '_' modeltype '.mat'], 'inds_lrs2',...
    ...
    if ~isfield(model, 'overlap'), model.overlap = OVLAP; end
    if isfield(model, 'sfx'), pos = enlargeWithContext(pos, model.sfx, model.sfy); end
    disp(' LRSPLIT2: getting subcategory membership info');
...
[mimg_init, mlab_init] = getMontagesForModel_latent(inds_init(:), inds_init(:), inds_lrs1, [], [], [], pos, [], numel(model.rules{model.start}));
[mimg_lrs1, mlab_lrs1] = getMontagesForModel_latent(inds_lrs1, inds_init(:), inds_lrs21, posscores_lrs1, possccalib_lrs1, lbbox_lrs1, pos, [], numel(model.rules{model.start}));
[mimg_lrs21, mlab_lrs21] = getMontagesForModel_latent(inds_lrs21, inds_lrs1, inds_lrs22, posscores_lrs21, possccalib_lrs21, lbbox_lrs21, pos, [], numel(model.rules{model.start}));
[mimg_lrs22, mlab_lrs22] = getMontagesForModel_latent(inds_lrs22, inds_lrs21, inds_lrs23, posscores_lrs22, possccalib_lrs22, lbbox_lrs22, pos, [], numel(model.rules{model.start}));
[mimg_lrs23, mlab_lrs23] = getMontagesForModel_latent(inds_lrs23, inds_lrs22, inds_lrs2, posscores_lrs23, possccalib_lrs23, lbbox_lrs23, pos, [], numel(model.rules{model.start}));
[mimg_lrs2, mlab_lrs2] = getMontagesForModel_latent(inds_lrs2, inds_lrs23, inds_lrs2, posscores_lrs2, possccalib_lrs2, lbbox_lrs2, pos, [], numel(model.rules{model.start}));

disp('Writing final montages');
mimg_cell = {mimg_init; mimg_lrs1; mimg_lrs21; mimg_lrs22; mimg_lrs23; mimg_lrs2};
mlab_cell = {mlab_init; mlab_lrs1; mlab_lrs21; mlab_lrs22; mlab_lrs23; mlab_lrs2};
