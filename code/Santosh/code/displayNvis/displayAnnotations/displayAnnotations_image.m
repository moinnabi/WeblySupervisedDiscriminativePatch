function displayAnnotations_image(VOCyear)
...
% load image set
[ids,gt]=textread([imgdir '/../ImageSets/Main/all.txt'],'%s %d'); 
%ids = mydir([imgdir '/*.jpg']);

imgid = 'sun_aaepnczurcpxcpgk';

    savename = [outdir '/' imgid];
        
    % read annotation
    %rec=PASreadrecord(sprintf(VOCopts.annopath,ids{f}));
    rec=PASreadrecord([imgdir '/../Annotations/' strtok(imgid,'.') '.xml']);
    
    % read image
    I=imread([imgdir '/' strtok(imgid,'.') '.jpg']);

    if 0 & rec.segmented        % read segmentations               
        [Sclass,CMclass]=imread(sprintf(VOCopts.seg.clsimgpath,imgid));
        [Sobj,CMobj]=imread(sprintf(VOCopts.seg.instimgpath,imgid));
    end
