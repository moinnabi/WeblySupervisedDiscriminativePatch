function mimg = myvisualizemodel_v4(model, components)
% Visualize a model.
...
%%%%%%%%%%%%%%
function dispim = visualizecomponent(model, c, nc, k)
...
if numparts > 0
    ...
    for i = 1:numparts
        ...
        % deformation model
        probex = size(p,2)/2; probey = size(p,1)/2;
        for y = 2*pad+1:size(p,1)-2*pad
            for x = 2*pad+1:size(p,2)-2*pad
                px = ((probex-x)/bs); py = ((probey-y)/bs);
                v = [px^2; px; py^2; py];
