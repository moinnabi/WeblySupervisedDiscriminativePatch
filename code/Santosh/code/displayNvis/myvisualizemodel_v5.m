function [mimg, mim, mlab] = myvisualizemodel_v5(model, components, layers)
...
k = 1;
[mim, mlab] = deal(cell(numel(components),1));
for i = components
    for layer = layers
        %mim{i} = visualizecomponent(model, i, length(components), k);
        %mim{i} = color(visualizecomponent(model, i, 1, 1));
        %visualizecomponent(model, i, length(layers)*length(components), k, layer);
        mim{i} = color(visualizecomponent(model, i, 1, 1, layer));
        mlab{i} = num2str(i);
        k = k+1;
    end
end
mimg = montage_list_w_text2(mim, mlab, 2, [], [], [1500 4500 3]);    

%%%%%%%%%%%%%%%%%%%
function dispim = visualizecomponent(model, c, nc, k, layer)
