function model = train_wsup_joint(cachedir, model, pos, neg, warp, randneg, iter, ...
...
if nargin < 14
    C = conf.training.C;    % 0.001        
    %{
    %C = C/20;   % lower C value as ~10X more comps/svs 
    C = C/(length(model.rules{model.start})/6);   % /6 as original C set wrt 6 comps
    disp(length(model.rules{model.start}));
    %}
    disp(['C value is ' num2str(C)]);
end

conf.training.lbfgs.options.corrections = 15;

%disp('reduce pos for now'); keyboard;
%pos = pos(1:100);

numpos = length(cat(1,pos(:).dataids));
% dont let too many negs compared to pos (roughtly 10 to 15x more negs than pos for the given param setting)
%max_num_examples = max(numpos*10, max_num_examples+numpos);
max_num_examples = max(numpos*15, max_num_examples+numpos);     
...
for t = 1:iter
    ...
    %%%{
    if ~cont || t > 1   % not continuing from prev it OR second+ round of latent
        ...
        if warp == 0
            ...
            %pos_loss(t,1) = C*mean(hinge);  % changed to mean as number of instances may change and it has to be normalized
        end
        ...
        if warp
            ...
            %[num_entries_added2, num_examples_added2, fusage2, component_usage2, scores2] ...
            ...
            %pos_loss(t,2) = C*mean(hinge);  % changed to mean as number of instances may change and it has to be normalized
            for tt = 1:t
            ...
            if t > 1 && pos_loss(t,2)*0.99999 > pos_loss(t,1) && numel(scores) == numel(pos_vals) % added 3rd if condition as number of instances may change and this check wouldnt make sense then
            ...
            if t > 1 && pos_loss(t,2)/pos_loss(t,1) > 0.999 && numel(scores) == numel(pos_vals) 
    ...
    for tneg = 1:negiter
        ...
        if datamine
            ...
            if randneg
                ...
                if ~cont && tneg == 1   % do this, if first round of dm AND you are not continuing from prev it
                    disp('neghard_joint_multi');
                    %[num_entries_added2, num_examples_added2, negpos2, fusage2, scores2, complete2] ...
                    %    = neghard_joint(tneg, negiter, model, neg, bytelimit, negpos, max_num_examples-num_examples);
                    [num_entries_added, num_examples_added, negpos, fusage, scores, complete] ...
                        = neghard_joint_multi(cachedir, tneg, negiter, model, neg, bytelimit, negpos, max_num_examples-num_examples);
                else
                    %disp('neghard');
                    %[num_entries_added, num_examples_added, negpos, fusage, scores, complete] ...
                    %    = neghard(tneg, negiter, model, neg, bytelimit, negpos, max_num_examples-num_examples);
                    disp('neghard_joint');
                    [num_entries_added, num_examples_added, negpos, fusage, scores, complete] ...
                        = neghard_joint(tneg, negiter, model, neg, bytelimit, negpos, max_num_examples-num_examples);
                end
        ...
        disp(['num threads for gradient coputation ' num2str(2*getNumberOfCores)]); 
        %obj_func = @(x) fv_obj_func(x, 2*pool_size);        
        obj_func = @(x) fv_obj_func(x, 2*getNumberOfCores);
        ...
        %{
        disp('loading pretrained model'); keyboard;
        fv_cache('load', '../dump/cache_debug_newintervalfixed.txt');  
        %}
        
        disp('check thresh, #negatives, Cval, corrections'); keyboard;      
        
        % Prepare the training examples residing in the cache for optimization
        fv_cache('ex_prepare');
        
        if ~exist([conf.paths.model_dir '/intermediateModels/' [model.class '_model_' tag '_' num2str(t) '_' num2str(tneg)] '.mat'], 'file')
        ...
        model.stats.slave_problem_time = [model.stats.slave_problem_time; th];
            ...
            disp('*******loading pretrained model******'); %keyboard;
            %w = getWfromPreviousModelInTrain([conf.paths.model_dir model_name '.mat']);
            tmpM = load([conf.paths.model_dir [model.class '_model_' tag '_' num2str(t) '_' num2str(tneg)] '.mat'], 'model');
            tmpblocks = fv_model_args(tmpM.model);
            w = cat(1, tmpblocks{:});
            [~, blah2] = fv_cache('gradient', w, 2*getNumberOfCores);            
        end
        
        [norms_before, bias_before] = computeModelComponentNorms(model);
        ...
        [norms_after, bias_after] = computeModelComponentNorms(model);
        disp([norms_before(:)'; bias_before(:)'; norms_after(:)'; bias_after(:)']);
        %disp('check norms to decide if to use neghard or neghard_joint for 2nd round of leanring'); keyboard;
        ...
        if ~isempty(pos_vals)       % added if in poslatent step, the detector just loses all its instances
            model.thresh = pos_vals(ceil(length(pos_vals)*0.05));
        else
            disp('no positives, a bad ngram');
            model.thresh = 0;
        end
        ...
        fprintf('Finished training %s (C = %.7f)\n', model_name, C);
        %}}}
