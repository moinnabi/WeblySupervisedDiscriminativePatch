function model = pascal_train_joint_nojoint_wsup(cls, objname, phrasenames, cachedir, year, fg_olap, borderoffset, jointCacheLimit, n_perngram)
...
if isdeployed, jointCacheLimit = str2num(jointCacheLimit); end
...
disp(['pascal_train_joint_nojoint_wsup(''' cls ''',''' objname ''','' phrasenames '',''' cachedir ''',''' year ''',''' num2str(fg_olap) ''',''' num2str(borderoffset) ''',''' num2str(jointCacheLimit) ''',''' num2str(n_perngram) ''')' ]);
...
try
    ...
    for ii = 1:numel(phrasenames)
        ...
        load([cachedir '/../' phrasenames{ii} '/' phrasenames{ii} '_mix_goodInfo'], 'compaps_full', 'compaps');
        for j=1:n_perngram
            if selcomps(j) == 1
    ...
    neg = pascal_data_wsup_neg(conf.pascal.VOCopts, 'val1_withLabels', year);  %'val' set
...
disp('Doing training');
try
    load([cachedir cls '_joint'], 'model');
    model;
catch
    seed_rand();
