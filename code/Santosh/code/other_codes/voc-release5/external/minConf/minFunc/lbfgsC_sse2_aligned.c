...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	for(ii=0;ii<nSteps;ii++)
	{
		...
		*/
		vecdot_odd(&temp, &y[nVars*ii], &s[nVars*ii], nVars);
		ro[ii] = 1/temp;
	}
	...
	*/
	veccpy_odd(&qq[nVars*nSteps], &g[0], nVars);
	...
	for(ii=nSteps-1;ii>=0;ii--)
	{
		...
		*/
		vecdot_odd(&alpha[ii], &s[nVars*ii], &qq[nVars*(ii+1)], nVars);
		alpha[ii] *= ro[ii];
		...
		*/
		vec3add_odd(&qq[nVars*ii], &qq[nVars*(ii+1)], &y[nVars*ii], -1*alpha[ii], nVars);
	}
	...
	for(ii=0;ii<nSteps;ii++)
	{
		...
		*/
		vecdot_odd(&beta[ii], &y[nVars*ii], &r[nVars*ii], nVars);
		beta[ii] *= ro[ii];
		...
		*/
		vec3add_odd(&r[nVars*(ii+1)], &r[nVars*ii], &s[nVars*ii], (alpha[ii]-beta[ii]), nVars);
	}
	...
	*/
	veccpy_odd(&d[0], &r[nVars*nSteps], nVars);
