function conf = voc_config_dsk(varargin)
...
conf = cv(conf, 'pascal.dev_kit', [DEVKIT_PATH '/VOC' ...
...
%conf = cv(conf, 'paths.model_dir', [conf.paths.base_dir '/' conf.project '/' conf.pascal.year '/']);
conf = cv(conf, 'paths.model_dir', [RESULTS_DIR '/']);
