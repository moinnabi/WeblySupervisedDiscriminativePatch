function model = pascal_train(cls, n, note, cachedir, year)
% Train a model.
...
disp(['pascal_train(''' cls ''',' num2str(n) ',''' note ''',''' cachedir ''',''' year ''')' ]);
...
try
  ...
  for i = 1:n
    ...
    inds{i} = lrsplit(models{i}, spos{i});
    % Train asymmetric root filter on one of these groups
    models{i} = train(models{i}, spos{i}(inds{i}), neg_large, true, true, 1, 1, ...
                      max_num_examples, fg_overlap, 0, false, ...
                      ['lrsplit1_' num2str(i)]);
  end
  save([cachedir cls '_lrsplit1'], 'models');
  %inds_init = getInitIndsForBaselrsplit(pos, posindex, inds, n);
  %save([cachedir cls '_displayInfo'], 'inds_init', 'inds', 'posindex');
end
