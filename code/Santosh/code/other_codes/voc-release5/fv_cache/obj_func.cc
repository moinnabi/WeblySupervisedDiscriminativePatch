 ...
 **/
// dsk: this function was used in sgd code, in new code, this is not needed; 
// gradient() computes this value
...
void gradient(double *obj_val_out, double *grad, const int dim, 
  ...
  #pragma omp parallel shared(grad_threads, grad_blocks, obj_vals)
  {
    ...
    for (int q = 0; q < num_examples; q++) {
      ...
      // each example can have multiple instances within it (for negatives?)	
      fv_iter I = i.begin;
      fv_iter belief_I = i.begin;
      double V = -INFINITY;
      double belief_score = 0;
