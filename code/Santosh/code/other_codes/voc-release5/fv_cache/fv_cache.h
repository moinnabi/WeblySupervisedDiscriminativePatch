...
struct fv {
  ...
  static int cmp_total(const fv &a, const fv &b) {
    ...
    // added by DSK on 17th May 2013
    // compare block lengths (this is redundant; num_blocks will be same if it
    // reached until here; you should just check block_labels
    //mexPrintf("checking num block\n");
    if (a.num_blocks < b.num_blocks){
      //mexPrintf("num blocks did not match in cmp_total");
      //mexPrintf("*");
      return -1;
    }
    else if (a.num_blocks > b.num_blocks){
      //mexPrintf("num blocks did not match in cmp_total");
      //mexPrintf("/");
      return 1;
    }

    //mexPrintf("checking block labels\n");
    for (int i = 0; i < a.num_blocks; i++){
      if (a.block_labels[i] < b.block_labels[i]){
        //mexPrintf("block labels did not match in cmp_total");
        //mexPrintf("+");
        return -1;
      }
      else if (a.block_labels[i] > b.block_labels[i]){
        //mexPrintf("block labels did not match in cmp_total");
        //mexPrintf("+");
        return 1;
      }
    }
    //mexPrintf("%d %d %d %d\n", a.key[0], a.key[1], a.key[2], a.block_labels[0]);
    //mexPrintf("%d %d %d %d\n", b.key[0], b.key[1], b.key[2], b.block_labels[0]);
    //mexPrintf("%d %d %d %d %d\n", a.key[0], a.key[1], a.key[2], a.num_blocks, a.feat_dim);
    //mexPrintf("%d %d %d %d %d\n", b.key[0], b.key[1], b.key[2], b.num_blocks, b.feat_dim);
    //mexPrintf("%d \n", a.block_labels[0]);
    //mexPrintf("%d \n", b.block_labels[0]);
    return 0;
  }
