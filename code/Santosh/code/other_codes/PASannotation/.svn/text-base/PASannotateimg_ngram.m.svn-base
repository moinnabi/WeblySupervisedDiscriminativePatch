function record=PASannotateimg_ngram(img,classes)
...
% Define keyboard accelerators for commonly used classes as lookup indices into the variable "classes"
accelerated(1)=1;  % objnmae
accelerated(2)=2;  % phrasename
...
while (boolmoreobjects==1)
    ...
    switch lower(char(but)),
        ...
        case 'c',
            ...
            record.objects(numobjects)=PASemptyobject;
            record.objects(numobjects).bbox=[xmin ymin xmax ymax];
            PASlabelobjfunc(char(classes(2)));
            %fprintf('\n Copied data for %s\n',classes{2});
