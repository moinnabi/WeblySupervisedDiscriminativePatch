function pascal_getNondupComps_noIslands(cachedir, phrasenames, datatype, data_year, numComp, modelname, wwwdispdir, wwwdispdir_part)    
% similar in spirit to getDiverseNgrams_fastImgCl (but at comp level)

try    

diary([cachedir '/diaryoutput_getNondupComps.txt']);    
disp(['pascal_getNondupComps_noIslands(''' cachedir ''','' phrasenames '',''' datatype ''',''' data_year ''',''' num2str(numComp) ''',''' modelname ''',''' wwwdispdir ''',''' wwwdispdir_part ''');' ]);
