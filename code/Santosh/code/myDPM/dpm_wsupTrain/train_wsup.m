function model = train_wsup(model, pos, neg, warp, randneg, iter, ...
...
for t = 1:iter
    ...
    if ~cont || t > 1
        ...
        if warp
            ...
            num_examples_added_pos = num_examples_added;
            if num_examples_added_pos == 0, fprintf('warning: no pos added\n'); end
    ...
    for tneg = 1:negiter
        ...
        if datamine
            ...
            if num_examples_added == 0  % added 21Sep13  
                if cont == false && num_examples_added_pos == 0
                    fprintf('no pos+neg examples added.\nModel is pretty much useless.\nSo just returning.\n');
                    return; % not "break"ing here as unnecesary negiters
                elseif cont == true
                   fprintf('no neg examples added.\nModel is pretty much useless.\nSo just returning.\n');
                   return;   
                end
            end
        else
