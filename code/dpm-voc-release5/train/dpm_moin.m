function model = dpm_moin(model, pos, neg, tag, C)
...
%following lines addedd by Moin
      [num_entries_added, num_examples_added] = negwarp(t, model, neg);
      fusage = num_examples_added;
      %component_usage = num_examples_added;
...
function [num_entries, num_examples] = negwarp(t, model, neg)
% assumption: the model only has a single structure rule 
% of the form Q -> F.
%rsize = model.filters(fi).size;

numneg = length(neg);
warped = warppos(model, neg);
fi = model.symbols(model.rules{model.start}.rhs).filter;
fbl = model.filters(fi).blocklabel;
obl = model.rules{model.start}.offset.blocklabel;
pixels = model.filters(fi).size * model.sbin / 2;
...
loss = 1;
for i = 1:numneg
  fprintf('%s %s: iter %d: warped Negative: %d/%d\n', ...
  ...
  dataid = i + 100000; % assumes < 100K foreground examples
  key = [dataid 0 0 0];
  bls = [obl; fbl] - 1;
  
  f = [model.features.bias; feat(:)];
